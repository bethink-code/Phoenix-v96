...
async function startSession(args) {
  ...
  void runLoop(session2, args).catch(async (err) => {
...
async function runLoop(session2, args) {
  const ctx = {
    goal: args.goal,
  ...
  for (let idx = 0; idx < args.maxIterations; idx++) {
    if (stopFlags.get(session2.id)) {
      ...
      await markStopped(session2.id, "aborted");
      return;
    }
  ...
  await db.update(autoresearchSessions).set({ status: "done", stoppedAt: /* @__PURE__ */ new Date() }).where(eq3(autoresearchSessions.id, session2.id));
}
async function markStopped(sessionId, status) {
  await db.update(autoresearchSessions).set({ status, stoppedAt: /* @__PURE__ */ new Date() }).where(eq3(autoresearchSessions.id, sessionId));
}
